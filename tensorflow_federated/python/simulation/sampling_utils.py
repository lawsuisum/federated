...
def build_sampling_fn(
    sample_range: Union[Sequence[Any], int],
    size: int,
    replace: bool = False,
  ...
  Args:
    sample_range: A 1-D array-like sequence or an integer, to be used as input
      to `np.random.choice`. If `sample_range` is a `Sequence`, then samples are
      generated randomly from the elements of the sequence. If `sample_range` is
      an integer, samples are generated from `range(sample_range)`.
  ...
  def sample(round_num, random_seed):
    ...
    return list(random_state.choice(sample_range, size=size, replace=replace))
